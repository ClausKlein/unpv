...
#define error(e)     \
    ...
    if ((rc = ga_echeck(hostname, servname, &hints)) != 0) error(rc);
    ...
    if (hostname != NULL && hostname[0] != '\0'
        ...
        if (aihead->ai_canonname == NULL) error(EAI_MEMORY);
    }

    /* 4now process the service name */
    if (servname != NULL && servname[0] != '\0') {
        if ((rc = ga_serv(aihead, &hints, servname)) != 0) error(rc);
    }
...
static int ga_unix(const char *path, struct addrinfo *hintsp,
    ...
    *result = ai;
    ...
    if (hintsp->ai_flags & AI_PASSIVE) unlink(path); /* OK if this fails */
